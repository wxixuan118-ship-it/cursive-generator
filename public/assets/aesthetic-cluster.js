(function(){
  ...
  function render(){
    var text=input.value||'Your Text';
    ...
    Array.prototype.forEach.call(grid.querySelectorAll('.cluster-copy'),function(btn){btn.addEventListener('click',function(){copy(shown[Number(btn.getAttribute('data-i'))].fn(input.value||'Your Text'),btn);});});
  }
  input.addEventListener('input',render);
  var reset=root.querySelector('.cluster-reset');if(reset)reset.addEventListener('click',function(){input.value='Your Text';input.focus();render();});
